...
import org.apache.spark.sql.SparkSession
    ...
    val data = spark.read.format("csv").option("inferSchema","True").option("header", "true").option("delimiter", ";").load("Data/bank-additional-full.csv")


    data.printSchema()
    val columnas = data.columns
    val columnasIndexadas = for(c <- columnas ) yield{ c+"Indexed"; }

    columnasIndexadas.show()
